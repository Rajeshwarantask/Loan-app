-- Rename previous_month_principal_received to additional_principal
-- and update the calculation to use current month instead of previous month

-- Step 1: Rename the column in monthly_loan_records table
ALTER TABLE monthly_loan_records 
RENAME COLUMN previous_month_principal_received TO additional_principal;

-- Step 2: Update the initialize_new_month function with new logic
...
BEGIN
  ...
  LOOP
    ...
    SELECT
      ...
      -- additional_principal: from CURRENT month's additional principal payments (not previous month)
      COALESCE((
...
COMMENT ON FUNCTION initialize_new_month IS 'Initializes monthly loan records for all users with active loans, calculating additional_principal from current month payments';
