-- Update sync_payment_to_monthly_record to use the new loan_payments schema
-- New schema columns: Monthly_Emi, interest_paid, principal_paid, amount, payment_date, etc.
-- Old columns removed: payment_type, penalty_component, interest_component, principal_component, subscription_component
...
BEGIN
  ...
  -- Using correct case-sensitive column name "Monthly_Emi" instead of monthly_emi
  -- Upsert monthly record using new schema columns
  ...
  DO UPDATE SET
    ...
    monthly_installment_income = monthly_loan_records.monthly_installment_income + COALESCE(NEW."Monthly_Emi", 0),
...
COMMENT ON FUNCTION sync_payment_to_monthly_record IS 'Syncs loan_payments to monthly_loan_records using new schema (Monthly_Emi, principal_paid, interest_paid)';
